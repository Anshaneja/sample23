...
export default function ProjectDesc() {
  ...
  const {id} = useParams();
  const project = arr1.filter((element)=>element.projectid === id);

  return (
    <div className=''>
      <div className="p-20 bg-blue-100 ">
      <div className="bg-white p-10 rounded-lg shadow-lg min-h-full px-10 mx-40">
      <h2 className="text-3xl font-bold mb-2 text-gray-800 text-center">{project[0].title}</h2>
      <p className="text-gray-700 py-2 text-lg font-medium">{project[0].description}</p>
      <div className="contain flex justify-center">
      
      <Link to ={"/terms"}>
      <button className='px-4 py-2 my-3 bg-blue-500 hover:bg-violet-600 font-semibold text-lg text-white rounded-md' >Donate</button>
      </Link>
      </div>
      </div>
      </div>
