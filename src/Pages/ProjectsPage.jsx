    ...
    <section className="md:h-full flex items-center text-gray-600">
        <div className="container px-5 py-5 mx-auto">
            <div className="text-center mb-12">
                <h1 className="text-4xl md:text-6xl text-gray-700 font-semibold">Punjabi University Upcoming Projects</h1>
            </div>
            <div className="flex flex-wrap -m-4 items-center">
            {articles.map((element)=>(
            <div className="p-4 sm:w-1/2 lg:w-1/3" key={element.url}>
            <ProjectItem title={element.title.slice(0, 31)} description={element.description.slice(0, 130)} imageUrl={element.urlToImage} projectid={element.projectid}/>
            </div>
            ))}
            </div>
        </div>
